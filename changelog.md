...
## 2026-08-31~09-01 — combat_v6 실패(교전 회피 재발) → 설치/해체 모드로 전환, combat_v7 완주
- **combat_v6 (MaxStep 구조 수정 후 재도전)**: 세션 5에서 발견한 "MaxStep 타임아웃 페널티(-1.5)가 죽음(-1)보다 나쁜" 역유인 문제를 `stepPenalty` -0.0005→-0.0003으로 축소해 해결(누적 상한 -0.9로 death penalty보다 약하게). 이동 시스템도 개선(벽에 막히면 미끄러지는 로직, 이후 제자리 떨림 발견해서 표면 밀어내기+정면충돌 시 정지로 추가 수정). 그럼에도 350만+ 스텝 동안 명중률 9~10%대(v5와 동일 수준)에서 정체, 보상이 주기적으로 -0.899/표준편차 0(=전원 완전 무교전 타임아웃)에 붕괴하는 패턴 반복 확인. 원인: 라운드가 팀 전멸까지 안 끝나는 구조라 정책이 "회피"에 빠지면 벗어날 유인이 약함 — **v6 중단, 체크포인트만 보존**(`results/combat_v6/`).
- **설치/해체(CS/Valorant 스타일) 모드로 전환 결정**: 라운드 타임리밋(숨으면 자동 패배)이라는 강제 장치가 필요하다고 판단. `MatchManager`에 폭탄 상태머신(설치/해체/폭발/타임아웃) 신규 구현, `CombatMLAgent` 관측 17→24개(역할/폭탄상태/사이트방향/시간압박/채널진행도), 행동에 설치·해체(interact) 액션 추가, 라운드 승패 보상 추가.
- **combat_v7 학습 중 실시간 디자인 반복**(사용자가 화면 보면서 지시): (1) 설치는 사이트 어디서나, 해체는 실제 설치 지점 기준 좁은 범위로 분리 — 처음엔 판정범위를 사이트와 동일하게 둬서 수비팀이 아무 데서나 해체하는 문제, 이후 판정범위를 사이트 중심 고정으로 잘못 설계했다가 "설치 위치가 다양해야 한다"는 지적으로 재수정. (2) 근접사격(밀착) 페널티 추가 — 판정범위가 좁아 양팀이 그냥 붙어서 쏘는 바람에 명중률이 실력과 무관하게 오르는 문제 발견 후 대응. (3) 수비팀은 사이트 근처, 공격팀은 사이트 반대편에서 시작하도록 스폰 로직 재설계(기존 씬 고정 스폰 대신 코드로 계산) — 반영 후 수비팀이 "선점 이점"으로 공격팀을 설치 전에 잡아내는 패턴(ATTACKERS ELIMINATED) 다수 확인, 밸런스 양호.
- **맵 2배 확장 시도 → 실패, 원복**: 스폰 거리를 더 벌리려고 `mapWidth/mapDepth` 52→104로 키웠으나, NavMesh 베이크가 15분 넘게 안 끝나서(브릿지 완전 응답없음) 원래 크기(52)로 되돌림. 거점을 진영 끝에 두는 설계는 유지(맵이 작아도 공격팀 이동거리 자체는 기존보다 늘어난 상태 - roundTimeLimit도 100→180초로 유지).
- **unity-cli 브릿지 자동 감시/복구 스크립트(`watchdog.ps1`) 신규 작성**: 초기 버전은 "브릿지 핑 실패 N회"를 기준으로 삼아서 정상적인 지연(무거운 시뮬레이션 중)까지 죽었다고 오판, 불필요한 재시작을 반복하는 문제가 있었음 → 판단 기준을 "학습 로그의 스텝 숫자가 실제로 멈췄는지"로 교체해 해결. 재시작 시마다 뜨는 "씬 백업 복구" 다이얼로그(taskkill이 비정상 종료로 기록되기 때문)도 재실행 전 임시 백업 파일 삭제로 자동 방지. PowerShell `Start-Process`의 몇 가지 함정(경로에 공백 있으면 인자가 쪼개짐, `-NoNewWindow`+콘솔없는 부모 조합에서 자식 프로세스 행)도 실측으로 발견해 수정.
- **combat_v7 400만 스텝 완주**: 명중률 22.4%(헤드샷 6.5%), 최근 라운드 기준 팀 밸런스 양호(공격/수비 승리 조건 다양하게 발생 - BOMB EXPLODED/BOMB DEFUSED/ATTACKERS ELIMINATED/TIME OUT 전부 관측됨). 목표(45~50%)는 아직 미달이지만 v5/v6 대비 뚜렷한 개선.
- 시간 관계상(새벽 3시반) combat_v8은 진행하지 않고 세션 종료. 다음 세션에서 이어서 반복 학습 또는 라운드/보상 추가 튜닝 검토.
